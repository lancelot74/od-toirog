...
export function ChartShare({chart,name}:{chart:Chart;name:string}){
  ...
  const lines=planet?[describe(planet)]:[...chart.planets.slice(0,2).map(describe),`Асцендент — ${chart.ascendant===null?'Тодорхойгүй':mn.signs[Math.floor(chart.ascendant/30)]}`];
