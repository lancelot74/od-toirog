...
export function AspectTable({aspects}:{aspects:Aspect[]}){return <div className="table-wrap"><table><thead><tr><th>Гаригууд</th><th>Холбоос</th><th>Өнцөг</th><th>Зөрүү</th></tr></thead><tbody>{aspects.map(a=><tr key={`${a.a}-${a.b}-${a.kind}`}><td><span className="planet-name"><AstroSymbol kind="planet" index={a.a} size={24}/>{mn.planets[a.a]}</span> · <span className="planet-name"><AstroSymbol kind="planet" index={a.b} size={24}/>{mn.planets[a.b]}</span></td><td>{mn.aspects[a.kind]}</td><td>{a.separation.toFixed(2)}°</td><td>{a.orb.toFixed(2)}°</td></tr>)}</tbody></table>{!aspects.length&&<p>Тогтоосон орбын хүрээнд гол холбоос олдсонгүй.</p>}</div>;}
