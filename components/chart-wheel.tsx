...
import { symbolHref, zodiacSlugs } from '@/lib/astro-assets';
...
export function ChartWheel({chart,onSelect}:{chart:Chart;onSelect:(planet:Planet)=>void}){
  ...
  return <svg viewBox="0 0 500 500" className="real-chart" role="group" aria-label="Төрсөн мөчийн натал зураг">
    ...
    {[[chart.ascendant,'ASC'],[chart.midheaven,'MC']].map(([lon,label])=>{if(lon===null)return null;const a=at(Number(lon),243);return <text key={label} x={a.x} y={a.y} textAnchor="middle" fontSize="8">{label}</text>;})}
  </svg>;
}
