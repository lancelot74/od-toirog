...
function Result({chart,profile}:{chart:Chart;profile:Profile}){
  ...
  const unavailable=chart.calculation?.id==='jpl-v0.1'?'Энэ аргад тооцохгүй':'Тодорхойгүй';
  return <>
    ...
    <div className="big-three">{[chart.planets[0],chart.planets[1]].map(p=><div key={p.id}><AstroSymbol kind="planet" index={p.id} size={40}/><p>{p.name}</p><b>{p.uncertain?'Тодорхойгүй':<ZodiacLink index={p.sign}/>}</b><small>{p.uncertain?'Лавлах байрлал':p.degree.toFixed(2)+'°'}</small></div>)}<div><span className="ascendant-marker" aria-hidden="true">ASC</span><p>Асцендент</p><b>{chart.ascendant===null?unavailable:<ZodiacLink index={Math.floor(chart.ascendant/30)}/>}</b></div></div>
    <div className="data-grid"><ChartWheel chart={chart} onSelect={setSelected}/><div>
      ...
      <section aria-live="polite"><h2 className="symbol-label"><AstroSymbol kind="planet" index={selected.id} size={40}/>{selected.name}</h2><p>{mn.signs[selected.sign]} · {selected.degree.toFixed(2)}° {selected.retrograde?'℞':''}</p>{selected.target_id!==undefined&&<p className="muted">JPL {selected.target_id} · {selected.target_kind==='body_center'?'Биетийн төв':'Гаригийн системийн массын төв'}<br/>Хурд: {selected.speed?.toFixed(6)}°/өдөр {selected.motion==='stationary'?'· Зогсонгих босгын дотор':''}</p>}{!selected.uncertain&&<ZodiacPortrait index={selected.sign}/>} {reading?<Reading reading={reading}/>:<p>Төрсөн цаг тодорхойгүй тул энэ гаригийн тайлал боломжгүй байна.</p>}</section>
    </div></div>
    ...
    <section role="tabpanel" id="chart-panel" aria-labelledby={`tab-${tabs.indexOf(tab)}`} tabIndex={0}>
      ...
      {tab==='Ордон'&&(chart.houses.length?chart.houses.map((lon,i)=><p key={i}>{i+1}-р ордон · {mn.signs[Math.floor(lon/30)]} {(lon%30).toFixed(2)}°</p>):<p>{chart.calculation?.id==='jpl-v0.1'?'JPL v0.1 нь ордны тооцоогүй. Swiss / Placidus аргыг сонгоно уу.':'Төрсөн цаг шаардлагатай.'}</p>)}
      {tab==='Холбоос'&&<AspectTable aspects={chart.aspects}/>}
      {tab==='Тайлбар'&&chart.readings.map(r=><Reading key={r.key+':'+r.headline} reading={r}/>)}
    </section>
    <p className="muted">{chart.ephemeris} · {chart.house_system==='unavailable'?'Ордон тооцоогүй':chart.house_system} · UTC {chart.utc}</p>
    <CalculationDetails calculation={chart.calculation}/>
