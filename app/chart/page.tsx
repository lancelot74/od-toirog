...
function Result({chart,profile}:{chart:Chart;profile:Profile}){
  ...
  return <>
    ...
    <div className="big-three">{[chart.planets[0],chart.planets[1]].map(p=><div key={p.id}><span>{mn.glyphs[p.id]}</span><p>{p.name}</p><b>{p.uncertain?'Тодорхойгүй':mn.signs[p.sign]}</b><small>{p.uncertain?'Лавлах байрлал':p.degree.toFixed(2)+'°'}</small></div>)}<div><span>↑</span><p>Асцендент</p><b>{chart.ascendant===null?'Тодорхойгүй':mn.signs[Math.floor(chart.ascendant/30)]}</b></div></div>
    <div className="data-grid"><ChartWheel chart={chart} onSelect={setSelected}/><div>
      ...
      <section aria-live="polite"><h2>{selected.name}</h2><p>{mn.signs[selected.sign]} · {selected.degree.toFixed(2)}° {selected.retrograde?'℞':''}</p>{reading?<Reading reading={reading}/>:<p>Төрсөн цаг тодорхойгүй тул энэ гаригийн тайлал боломжгүй байна.</p>}</section>
    </div></div>
    ...
    <section role="tabpanel" id="chart-panel" aria-labelledby={`tab-${tabs.indexOf(tab)}`} tabIndex={0}>
      {tab==='Гаригууд'&&<div className="table-wrap"><table><thead><tr><th>Гариг</th><th>Орд</th><th>Хэм</th><th>Ордон</th></tr></thead><tbody>{chart.planets.map(p=><tr key={p.id}><td>{p.name} {p.retrograde?'℞':''}</td><td>{mn.signs[p.sign]} {p.uncertain?'(лавлах)':''}</td><td>{p.degree.toFixed(2)}°</td><td>{p.house??'—'}</td></tr>)}</tbody></table></div>}
      {tab==='Орд'&&mn.signs.map((sign,i)=><p key={sign}>{sign}: {chart.planets.filter(p=>p.sign===i&&!p.uncertain).map(p=>p.name).join(', ')||'—'}</p>)}
