...
function Result({chart,profile}:{chart:Chart;profile:Profile}){
  ...
  return <>
    ...
    <div className="big-three">{[chart.planets[0],chart.planets[1]].map(p=><div key={p.id}><AstroSymbol kind="planet" index={p.id} size={40}/><p>{p.name}</p><b>{p.uncertain?'Тодорхойгүй':<ZodiacLink index={p.sign}/>}</b><small>{p.uncertain?'Лавлах байрлал':p.degree.toFixed(2)+'°'}</small></div>)}<div><span className="ascendant-marker" aria-hidden="true">ASC</span><p>Асцендент</p><b>{chart.ascendant===null?'Тодорхойгүй':<ZodiacLink index={Math.floor(chart.ascendant/30)}/>}</b></div></div>
